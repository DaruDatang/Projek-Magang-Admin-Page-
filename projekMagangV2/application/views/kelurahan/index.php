<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center">
        ...
        <?php
            ...
            $kec_param = $this->input->get('kode_kecamatan') ? htmlspecialchars($this->input->get('kode_kecamatan')) : '';
        ?>

        <form method="get" id="filterFormProv" class="me-2">
        ...
        <form method="get" id="filterFormKab" class="me-2">
            <input type="hidden" name="q" value="<?= $q_param; ?>">
            <input type="hidden" name="order" value="<?= $order_param; ?>">
            <input type="hidden" name="sort" value="<?= $sort; ?>">
        ...
        <form method="get" id="filterFormKec" class="me-2">
            <input type="hidden" name="q" value="<?= $q_param; ?>">
            <input type="hidden" name="order" value="<?= $order_param; ?>">
            <input type="hidden" name="sort" value="<?= $sort; ?>">
            <input type="hidden" name="kode_provinsi" value="<?= $prov_param; ?>">
            <input type="hidden" name="kode_kabupaten" value="<?= $kab_param; ?>">
            <select name="kode_kecamatan" class="form-select form-select-sm" onchange="this.form.submit()" title="Filter berdasarkan kecamatan" <?= (empty($selected_kabupaten)) ? 'disabled' : ''; ?>>
                <option value="">-- Semua Kecamatan --</option>
                <?php foreach($kecamatan_list as $kc): ?>
                    <option value="<?= $kc['kode']; ?>" <?= ($kc['kode'] == $selected_kecamatan) ? 'selected' : ''; ?>>
                        <?= $kc['nama']; ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <form method="get" class="me-2">
            ...
            <input type="hidden" name="kode_provinsi" value="<?= $prov_param; ?>">
            <input type="hidden" name="kode_kabupaten" value="<?= $kab_param; ?>">
            <input type="hidden" name="kode_kecamatan" value="<?= $kec_param; ?>">
            <div class="input-group input-group-sm">
                <input type="text" name="q" class="form-control" placeholder="Cari kelurahan"
                    value="<?= htmlspecialchars($q_param); ?>" style="width: 120px;">
        ...
        <a href="<?= base_url('kelurahan/add'); ?>" class="btn btn-primary btn-sm">
        ...
        <table class="table table-bordered table-striped align-middle">
            ...
            <tbody>
                <?php if (empty($kelurahan)): ?>
                    <tr><td colspan="6" class="text-center">Tidak ada data kelurahan</td></tr>
                <?php else: ?>
                    <?php foreach($kelurahan as $k): ?>
                        <tr>
                            <td><?= htmlspecialchars($k['kode']); ?></td>
                            <td><?= htmlspecialchars($k['nama']); ?></td>
                            <td><?= htmlspecialchars($k['kecamatan_nama']); ?></td>
                            <td><?= htmlspecialchars($k['kabupaten_nama']); ?></td>
                            <td><?= htmlspecialchars($k['provinsi_nama']); ?></td>
                            <td>
                                <a href="<?= base_url('kelurahan/edit/'.$k['kode']); ?>" class="btn btn-sm btn-warning" title="Edit">
                                    <i class="fas fa-pencil"></i>
                                </a>
                                <a href="<?= base_url('kelurahan/delete/'.$k['kode']); ?>" onclick="return confirm('Hapus data ini?')" class="btn btn-sm btn-danger" title="Hapus">
